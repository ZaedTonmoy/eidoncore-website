(function () {
  ...
  function init() {
    ...
    /* Premium homepage motion: element assembly, letter reveals, 40% trigger */
      ...
      document.querySelectorAll('.word-reveal').forEach(el => {
        ...
        const wrapText = (text) => {
          ...
          return chars.map(char => {
            ...
            return `<span class="wr-char" style="--w-i:${unitIndex++}" aria-hidden="true">${safeChar}</span>`;
          }).join('');
        };

        childNodes.forEach(node => {
          if (node.nodeType === Node.TEXT_NODE) {
            newContent += wrapText(node.textContent);
          } else if (node.nodeType === Node.ELEMENT_NODE) {
            const clone = node.cloneNode(false);
            clone.innerHTML = wrapText(node.textContent);
    ...
    function initHeaderMotion() {
      ...
      if (!nav || nav.dataset.motionInit) return;
      nav.dataset.motionInit = 'true';
      nav.classList.add('nav-assembled');

      const pieces = [
        nav.querySelector('.nav-logo'),
        ...nav.querySelectorAll('.nav-links > li'),
        ...nav.querySelectorAll('.nav-actions .btn'),
        nav.querySelector('.nav-toggle')
      ].filter(Boolean);

      pieces.forEach((piece, index) => {
        piece.classList.add('nav-motion-piece');
        piece.style.setProperty('--i', index);
      });

      requestAnimationFrame(() => nav.classList.add('in'));
    }

    function initHomepageChoreography() {
      const home = document.querySelector('body > main #hero');
      if (!home || document.documentElement.dataset.homeMotionInit) return;
      document.documentElement.dataset.homeMotionInit = 'true';

      const addReveal = (selector, className = 'reveal', delay = null) => {
        document.querySelectorAll(selector).forEach(el => {
          el.classList.add(...className.split(/\s+/).filter(Boolean));
          if (delay !== null) el.style.setProperty('--motion-delay', `${delay}ms`);
        });
      };

      addReveal('.hero-header-text .badge', 'reveal motion-eyebrow', 80);
      addReveal('.hero-header-text .sub', 'reveal motion-copy', 760);
      addReveal('.hero-header-text .cta-row', 'reveal motion-actions', 940);
      addReveal('.hero-header-text .hero-trust-note', 'reveal motion-logos', 1100);
      addReveal('.hero-showcase-frame', 'reveal motion-visual', 520);
      addReveal('.hero-bottom-metrics', 'reveal-stagger motion-stats', 1040);

      document.querySelectorAll('.section-head, .head-row, .eyebrow-wrap').forEach(sectionHead => {
        sectionHead.classList.add('reveal', 'motion-sequence');
        sectionHead.querySelectorAll('.eyebrow, .kicker').forEach(el => el.classList.add('motion-eyebrow'));
        sectionHead.querySelectorAll('h1, h2').forEach(el => el.classList.add('word-reveal', 'motion-heading'));
        sectionHead.querySelectorAll('p, .sub').forEach(el => el.classList.add('motion-copy'));
        sectionHead.querySelectorAll('.btn, .btn-pill, .view-all-link, a').forEach(el => el.classList.add('motion-action'));
      });

      document.querySelectorAll('.trust-bento .container.wrap').forEach(el => el.classList.add('motion-sequence'));
      document.querySelectorAll('.trust-bento .kicker').forEach(el => el.classList.add('motion-eyebrow'));
      document.querySelectorAll('.trust-bento .tm-number, .trust-bento .tile-title').forEach(el => el.classList.add('word-reveal'));

      document.querySelectorAll('.features-grid-section .head-row h2, #workflow-loop h2').forEach(el => {
        el.classList.add('word-reveal', 'motion-heading');
      });
      document.querySelectorAll('#workflow-loop .loop-eyebrow, #workflow-loop .loop-lede, #workflow-loop .loop-note, #workflow-loop .orbit-wrap, #workflow-loop .loop-timeline').forEach((el, index) => {
        el.classList.add(index === 0 ? 'motion-eyebrow' : index > 2 ? 'motion-visual' : 'motion-copy');
      });
      document.querySelector('#workflow-loop .loop-layout')?.classList.add('reveal', 'motion-sequence');
      document.querySelector('#how-it-works .hw-tabs')?.classList.add('reveal-stagger');
      document.querySelector('#how-it-works .hw-device')?.classList.add('reveal', 'motion-visual');
      document.querySelector('.why-eidoncore-section .stack-wrap')?.classList.add('reveal-stagger');

      document.querySelectorAll('.tm-logos, .hero-bottom-metrics').forEach(group => {
        Array.from(group.children).forEach((child, index) => child.style.setProperty('--i', index));
      });
    }

    /* Eye-soothing element-by-element scroll reveal system */
    function initScrollReveal() {
      initHeaderMotion();
      initHomepageChoreography();
      initWordReveal();

      // Auto-assign --i stagger indices to children in grids/stagger containers if not present
      document.querySelectorAll('.reveal-stagger, .features-modules-grid, .pricing-grid, .testimonials-grid, .trust-bento .grid, .tools-grid, .compare-wrap, .stack-wrap, .tm-logos, .hw-tabs').forEach(grid => {
      ...
      const pending = new Set(Array.from(revealEls).filter(el => !el.classList.contains('in')));
      let revealTicking = false;

      function checkReveal() {
        revealTicking = false;
        const winHeight = window.innerHeight || document.documentElement.clientHeight;
        const triggerY = winHeight * 0.4;
        pending.forEach(el => {
          const rect = el.getBoundingClientRect();
        ...
        if (!pending.size) {
          ...
          if (revealPoll) window.clearInterval(revealPoll);
        }
      }
      ...
      scheduleRevealCheck();
      const revealPoll = window.setInterval(scheduleRevealCheck, 250);
